...
void disList(List *list) {
    ...
    printf("List Barang: \n");
    printf("\n===========================================================================\n");
    printf("| %-20s | %-15s | %-10s | %-5s |\n", "Nama Barang", "Deskripsi", "Harga (Rp)", "Stok");
    printf("===========================================================================\n");
...
void addStock(List *list) {
    ...
    // Read the file and store lines in a temporary buffer
    FILE *file = fopen("jualan.txt", "r");
    if (file == NULL) {
        printf("Gagal membuka file.\n");
        return;
    ...
    char line[256];
    char buffer[10000] = ""; // Temporary buffer to store file content
    while (fgets(line, sizeof(line), file) != NULL) {
        Node temp;
        sscanf(line, "%[^|]|%[^|]|%f|%d", temp.info, temp.detail, &temp.price, &temp.available);
        if (strcmp(temp.info, name) == 0) {
            // Update the line with new stock value
            sprintf(line, "%s|%s|%.2f|%d\n", found->info, found->detail, found->price, found->available);
        }
        strcat(buffer, line);
    }
    fclose(file);
